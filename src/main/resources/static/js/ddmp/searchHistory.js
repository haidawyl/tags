...
var TableInit = function () {
    ...
    function operateFormatter(value, row, index) {
        ...
        if(row.isDelete == 0){
            buttonStr +=  '&nbsp;&nbsp;<button onclick="deleteHistory(' + row.id + ')" type="button" class="btn btn-default btn-sm fa fa-trash-o">删除</button>';
        }
        if(row.isFav == 0){
            buttonStr += '&nbsp;&nbsp;<button type="button" class="btn btn-default btn-sm fa fa-tag"  onclick="showModal(' + row.id + ')">收藏</button>'
        }else{
            buttonStr += '&nbsp;&nbsp;<span>已收藏</span>'
        }
        return [
            buttonStr
        ].join('');
